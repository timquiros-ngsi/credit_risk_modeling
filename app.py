...
def prepare_model_input(feats):
    ...
    loanptrate = np.log(feats['term'] + 1)
    loan_type = labels_dict['loantype'][feats['type']]
    age = np.log(feats['age'] + 1)
    loan_amount = np.log(feats['amount'] + 1)
    me_svc_stat = labels_dict['me_svc_stat'][feats['svc_stat']]
    gross_pay = np.log(feats['gross_pay'] + 1)
    pnp_bill_mode = labels_dict['pnpbillmode'][feats['bill_mode']]
...
def load_data():
    x_test = pd.read_csv(
        ...
        usecols=['loanptrate','loantype','me_age','me_svc_stat','loanamt','grosspay1','pnpbillmode']
    )
    y_test = pd.read_csv(
...
def main():
    ...
    #Age
    st.sidebar.subheader("How old is the client?")
    age = st.sidebar.number_input("Age", 18, 120, step=1, key='age')
    ...
    loan_amount = st.sidebar.number_input("Loan Amount", 5000.0, 10000000.0, step=1000.0, key='loan_amount')
    ...
    feat_input = {
        ...
        'type': loan_type,
        'age': age,
        'svc_stat': me_svc_stat,
        'amount': loan_amount,
        'gross_pay': gross_pay,
        'bill_mode': pnp_bill_mode
    }

    X = prepare_model_input(feat_input)
